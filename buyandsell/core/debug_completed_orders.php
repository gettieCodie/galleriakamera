...
echo "=== All Orders with Status ===\n";
$result = $conn->query('SELECT OrderID, Status FROM Orders ORDER BY OrderID DESC LIMIT 20');
if ($result) {
    while($row = $result->fetch_assoc()) {
        echo "OrderID: " . $row['OrderID'] . ", Status: " . $row['Status'] . "\n";
    }
} else {
    echo "Error: " . $conn->error . "\n";
}

echo "\n=== Top Selling Cameras (FROM COMPLETED ORDERS ONLY) ===\n";
$sql = "SELECT 
